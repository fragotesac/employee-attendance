(function( $ ){
	$.fn.multiple_emails = function(options) {
		...
		return this.each(function() {
			...
			var $orig = $(this);
			var $list = $('<ul class="multiple_emails-ul" style="margin-top: 3%;"/>'); // create html elements - list of email addresses as unordered list
			...
			var $input = $('<input type="text" style="position: absolute !important;width:96% !important;height: 30px;border-bottom: 1px solid #ced4da !important;" placeholder="Escriba un nuevo email aquí..." id="inp-email" class="multiple_emails-input text-left"/>').on('keyup', function(e) { // input
			...
			var $container = $('<div style="overflow-y: scroll;height: 142px !important;" class="multiple_emails-container" />').click(function() { /*$input.focus();*/ } ); // container div
			...
			function display_email(t, dupEmailCheck) {
				...
				for	(var i = 0; i < arr.length; i++) {
					...
					if ( dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1 ) {
						...
						$list.append($('<li class="multiple_emails-email"><span class="email_name lb_email spanDetalle" data-permail="'+ arr[i].toLowerCase() +'" data-email="' + arr[i].toLowerCase() + '">' + arr[i] + '</span></li>')
							  .prepend($(deleteIconHTML)
								   .click(function(e) { $(this).parent().remove(); refresh_emails(); e.preventDefault(); })
							  )
						);
